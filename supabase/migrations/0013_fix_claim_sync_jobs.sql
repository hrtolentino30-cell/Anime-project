create or replace function public.claim_sync_jobs(p_limit int default 5)
...
begin
  ...
  where q.id in (select sq.id from public.sync_queue sq where sq.status='pending' and sq.available_at <= now() order by sq.priority asc, sq.created_at asc for update skip locked limit greatest(1, least(p_limit,20)))
  returning q.*;
end $$;
