...
export async function AdminDashboard(){
  ...
  return <div className="adminPage">
    <div className="statsGrid"><Stat label="Anime" value={animeResult.count??0}/><Stat label="Episodes" value={episodeResult.count??0}/><Stat label="Open queue" value={openResult.count??0}/><Stat label="Needs attention" value={actionable.length}/><Stat label="Latest episode" value={latestEpisode?`${(latestEpisode.anime as any)?.title??'—'} · EP ${latestEpisode.episode_number}`:'—'}/><Stat label="Latest sync" value={lastRun?.finished_at?new Date(lastRun.finished_at).toLocaleString():'—'}/></div>
    <AdminControls/>
