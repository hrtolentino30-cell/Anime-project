...
export function Player({episodeId,animeId,sources,userId,initialPosition=0,nextEpisodeId}:{episodeId:string;animeId:string;sources:VideoSource[];userId:string|null;initialPosition?:number;nextEpisodeId?:string}){
  ...
  useEffect(()=>{
    ...
    const onMeta=()=>{if(initialPosition>0&&Number.isFinite(video.duration)&&initialPosition<video.duration-15){video.currentTime=initialPosition;setNotice(`Resumed from ${formatTime(initialPosition)}.`)}};
    const onError=()=>failover();video.addEventListener('loadedmetadata',onMeta);video.addEventListener('error',onError);
    ...
    return()=>{cancelled=true;video.removeEventListener('loadedmetadata',onMeta);video.removeEventListener('error',onError);hls?.destroy();video.pause();video.removeAttribute('src');video.load()};
  },[source,initialPosition,sources]);

  useEffect(()=>{
    if(!userId)return;const db=createSupabaseBrowserClient();
    const save=async()=>{const video=videoRef.current;if(!video||!Number.isFinite(video.duration)||video.duration<=0)return true;const [progress,history]=await Promise.all([db.from('playback_progress').upsert({user_id:userId,episode_id:episodeId,anime_id:animeId,position_seconds:video.currentTime,duration_seconds:video.duration,completed:video.currentTime/video.duration>0.92,updated_at:new Date().toISOString()},{onConflict:'user_id,episode_id'}),db.from('watch_history').upsert({user_id:userId,anime_id:animeId,episode_id:episodeId,watched_at:new Date().toISOString()},{onConflict:'user_id,episode_id'})]);const error=progress.error??history.error;setSaveError(error?'Progress could not be saved.':'');return !error};
    const timer=setInterval(()=>void save(),10000);const video=videoRef.current;const onPause=()=>{void save()};const onEnded=()=>{void (async()=>{await save();if(autoNext&&nextEpisodeId){setNotice('Episode complete. Loading next episode…');setTimeout(()=>location.assign(`/watch/${nextEpisodeId}`),700)}})()};video?.addEventListener('pause',onPause);video?.addEventListener('ended',onEnded);
