...
export function Player({episodeId,animeId,sources,userId,initialPosition=0,nextEpisodeId}:{episodeId:string;animeId:string;sources:VideoSource[];userId:string|null;initialPosition?:number;nextEpisodeId?:string}){
  ...
  useEffect(()=>{
    if(!sources.some(s=>s.id===selected))setSelected(sources[0]?.id??'');
  },[sources,selected]);

  useEffect(()=>{
    if(!userId)return;
    const db=createSupabaseBrowserClient();
    void db.from('watch_history').upsert({user_id:userId,anime_id:animeId,episode_id:episodeId,watched_at:new Date().toISOString()},{onConflict:'user_id,episode_id'}).then(({error})=>{if(error)setSaveError('Watch history could not be saved.')});
  ...
  useEffect(()=>{
    ...
    const failover=()=>{
      ...
      if(next){setNotice(`Source unavailable. Trying ${next.server_name}…`);setSelected(next.id)}
      else setNotice('This episode has no other healthy source right now.');
    };
    video.pause();
    ...
    const onMeta=()=>{
      if(initialPosition>0&&Number.isFinite(video.duration)&&initialPosition<video.duration-15){
        video.currentTime=initialPosition;
        setNotice(`Resumed from ${formatTime(initialPosition)}.`);
      }
    };
    ...
    if(source.source_type==='hls'){
      if(video.canPlayType('application/vnd.apple.mpegurl'))video.src=url;
        ...
        if(cancelled||!Hls.isSupported()){if(!cancelled)failover();return}
        const instance=new Hls({enableWorker:true,capLevelToPlayerSize:true,maxBufferLength:30});
        hls=instance;
        instance.on(Hls.Events.ERROR,(_event:any,data:any)=>{if(data?.fatal)failover()});
  ...
  useEffect(()=>{
    ...
    const save=async()=>{
      ...
      const [progress,history]=await Promise.all([
        db.from('playback_progress').upsert({user_id:userId,episode_id:episodeId,anime_id:animeId,position_seconds:video.currentTime,duration_seconds:video.duration,completed:video.currentTime/video.duration>0.92,updated_at:new Date().toISOString()},{onConflict:'user_id,episode_id'}),
        db.from('watch_history').upsert({user_id:userId,anime_id:animeId,episode_id:episodeId,watched_at:new Date().toISOString()},{onConflict:'user_id,episode_id'}),
      ]);
      const error=progress.error??history.error;
      setSaveError(error?'Progress could not be saved.':'');
      return !error;
    };
    const timer=setInterval(()=>void save(),10000);
    const video=videoRef.current;
    const onPause=()=>{void save()};
    const onEnded=()=>{void (async()=>{await save();if(autoNext&&nextEpisodeId){setNotice('Episode complete. Loading next episode…');setTimeout(()=>location.assign(`/watch/${nextEpisodeId}`),700)}})()};
  ...
  return <div className="playerShell">
    <div className="playerFrame">{embed?<iframe src={url??''} title={`${source.server_name} player`} allow="autoplay; encrypted-media; picture-in-picture; fullscreen" sandbox="allow-scripts allow-same-origin allow-presentation" allowFullScreen referrerPolicy="no-referrer" loading="lazy"/>:<video ref={videoRef} controls playsInline preload="metadata"/>}</div>
    <div className="servers" aria-label="Playback servers"><span>Servers</span>{sources.map((s,index)=><button key={s.id} type="button" onClick={()=>{setNotice('');setSelected(s.id)}} className={s.id===source.id?'active':''} aria-pressed={s.id===source.id} title={`Play from ${s.server_name}`}><small>{s.language??'default'}</small>{s.server_name}{s.quality&&<em>{s.quality}</em>}{index>0&&s.source_type!=='hls'&&s.source_type!=='mp4'&&<em>fallback</em>}</button>)}{nextEpisodeId&&<label className="autoNext"><input type="checkbox" checked={autoNext} onChange={e=>setAutoNext(e.target.checked)}/>Auto-next</label>}</div>
