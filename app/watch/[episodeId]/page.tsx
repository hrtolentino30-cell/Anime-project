...
export default async function WatchPage({params}:{params:Promise<{episodeId:string}>}){
  ...
  const jsonLd={'@context':'https://schema.org','@type':'TVEpisode',name:`${anime?.title??'Anime'} Episode ${episode.episode_number}`,episodeNumber:Number(episode.episode_number),partOfSeries:anime?{'@type':'TVSeries',name:anime.title,url:`https://animori.vercel.app/anime/${anime.slug}`}:undefined,url:`https://animori.vercel.app/watch/${episode.id}`};
