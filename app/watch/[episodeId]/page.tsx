...
export default async function WatchPage({params}:{params:Promise<{episodeId:string}>}){
  ...
  const breadcrumbs={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'Home',item:'https://www.animori.bond/'},{'@type':'ListItem',position:2,name:anime?.title??'Anime',item:`https://www.animori.bond/anime/${anime?.slug}`},{'@type':'ListItem',position:3,name:`Episode ${episode.episode_number}`,item:`https://www.animori.bond/watch/${episode.id}`}]};
  return <div className="watchPage pageWidth"><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(jsonLd).replace(/</g,'\\u003c')}}/><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(breadcrumbs).replace(/</g,'\\u003c')}}/>
    ...
    <Player episodeId={episode.id} animeId={episode.anime_id} animeSlug={anime?.slug??''} animeTitle={anime?.title??'Anime'} episodeNumber={episode.episode_number} episodeTitle={episode.title} backdropUrl={anime?.banner_url || anime?.poster_url || null} sources={sources as any} siblings={siblings as any} userId={userId} initialPosition={Number(progress?.position_seconds??0)}/>
