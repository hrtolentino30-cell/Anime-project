...
- Smoking Behind the Supermarket episode 11: legacy run 35353256872 accepted video 4570709826547901, then timed out while Meta was processing. Reconciliation returned Meta unsupported-get (missing object or permissions). The queue retains that video ID with upload_started=true and status=failed for review, blocking canonical reupload and avoiding queue starvation. Existing duplicate posts have not been deleted.
...
Proxy: facebook-upload-proxy version 9, GitHub OIDC restricted to the exact main-branch workflow.
...
Terminal Meta errors are quarantined. Accepted-upload recovery is capped at six attempts so a permanently unavailable video cannot starve newer releases.
