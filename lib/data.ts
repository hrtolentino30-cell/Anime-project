...
const getCachedAnimeBySlug=unstable_cache(async(slug:string)=>{
  const db=createPublicCatalogClient();
  const {data:anime,error}=await db.from('anime').select('id,slug,title,title_english,title_japanese,description,poster_url,banner_url,type,status,season,year,rating,duration,latest_episode,total_episodes,source_updated_at,created_at,updated_at,anime_genres(genres(id,name,slug)),anime_studios(studios(id,name,slug)),anime_titles(kind,title)').eq('slug',slug).single();
  if(error||!anime)return null;
  const [{data:episodes},{data:relations}]=await Promise.all([
    db.from('episodes').select('id,anime_id,episode_number,title,thumbnail_url,air_date,created_at,updated_at').eq('anime_id',anime.id).order('episode_number',{ascending:true}),
...
},['animori-anime-detail-v3'],{revalidate:300});
...
},['animori-watch-core-v3'],{revalidate:60});
